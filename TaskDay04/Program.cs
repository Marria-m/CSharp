...
namespace ProblemDay04
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            #region Question of Part02 (Enum)
            Console.Write("Enter day number (1-7): ");
            int dayNum;
            bool validDay = int.TryParse(Console.ReadLine(), out dayNum);

            if (validDay && dayNum >= 1 && dayNum <= 7)
            {
            ...
            {
                Console.WriteLine("not right, enter number 1-7.");
            }
            Console.WriteLine();

            // Using Enum.Parse
            Console.Write("Enter day number again (1-7): ");
            string input = Console.ReadLine();
            try
            {
                DayOfWeek dayParsed = (DayOfWeek)Enum.Parse(typeof(DayOfWeek), input);
                Console.WriteLine($"Day using Enum.Parse: {dayParsed}");
            }
            catch
            {
            ...
            // Question Answer:
            // If user enters value outside 1-7:
            // - Direct cast will still work but give unexpected enum name
            // - For example: entering 10 will cast to (DayOfWeek)10 which displays "10" not a valid day
            // - Enum.Parse will throw exception if string doesn't match enum value
            // - Need validation or use Enum.IsDefined() to check if value is valid
